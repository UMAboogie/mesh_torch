...
class Model(nn.Module):
    ...
    def _build_graph(self, inputs, is_training):
        ...
        node_features = torch.cat((velocity, density, node_type), dim=-1)
    ...
    def loss(self, inputs):
        ...
        cur_density = inputs['density']
        target_density = inputs['target_density']
        target_density_change = target_density - cur_density
        ground_truth = torch.cat([target_velocity_change, target_density_change], dim=1)
        target_normalized = self._output_normalizer(ground_truth).to(device)
    ...
    def _update(self, inputs, per_node_network_output):
        ...
        velocity_update, density_update = torch.split(self._output_normalizer.inverse(per_node_network_output), [2,1], dim=1)
        # integrate forward
        cur_velocity = inputs['velocity']
        cur_density = inputs['density']
        return cur_velocity + velocity_update, cur_density + density_update
    ...
    def save_model(self, path, epoch=None):
        if epoch == None:
            ...
            torch.save(self._edge_normalizer, path + "/_mesh_normalizer"+str(epoch)+".pth")
