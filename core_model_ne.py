...
class GraphNetBlock(nn.Module):
    ...
    def _update_edge_features(self, node_features, edge_set):
        ...
        recievers = edge_set.recievers.to(device)
        sender_features = torch.index_select(input=node_features, dim=0, index=senders)
        reciever_features = torch.index_select(input=node_features, dim=0, index=recievers)
        features = [sender_features, reciever_features, edge_set.features]
    ...
    def _update_node_features(self, node_features, edge_sets):
        ...
        for edge_set in edge_sets:
            features.append(
                self.unsorted_segment_operation(edge_set.features, edge_set.recievers, num_nodes))
        features = torch.cat(features, dim=-1)
...
class Encoder(nn.Module):
    ...
    def forward(self, graph):
        ...
        for index, edge_set in enumerate(graph.edge_sets):
            if edge_set.name == "mesh_edges":
                feature = torch.index_select(node_latents,0,edge_set.senders)+torch.index_select(node_latents,0,edge_set.recievers)
